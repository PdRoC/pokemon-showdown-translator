// Diccionarios (Copia aquí los objetos AbilitiesDico y translations del original)
const AbilitiesDico = {
    // Habilidades
...
// Función optimizada de traducción
function traducirNodo(nodo) {
    if (nodo.nodeType === Node.TEXT_NODE) {
        let texto = nodo.textContent.trim();
        
        // Traducción directa de interfaz
        if (translations[texto]) {
            nodo.textContent = translations[texto];
        }
        
        // Traducción de habilidades (Ejemplo rápido)
        for (let [en, es] of Object.entries(AbilitiesDico)) {
            if (texto.includes(en)) {
                nodo.textContent = texto.replace(en, es);
            }
        }
    } else {
        // Recorrer hijos para no perder nodos anidados
        nodo.childNodes.forEach(traducirNodo);
    }
}

// Observador de mutaciones para SPA (Single Page Application)
const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) { // Elementos HTML
                traducirNodo(node);
            }
        });
    });
...
// Iniciar la observación en el cuerpo del documento
